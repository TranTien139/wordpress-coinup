...
if ( ! class_exists( 'BFI_Thumb_1_3' ) ) {
    ...
    class BFI_Thumb_1_3 {
        ...
        public static function thumb( $url, $params = array(), $single = true ) {
            ...
            $image_name_path =  strtolower(preg_replace("/[^a-zA-Z0-9]+/", "", "{$dst_rel_path}-{$suffix}.{$ext}")); //str_replace("-","","{$dst_rel_path}-{$suffix}.{$ext}");
            ...
            if ( defined( 'BFITHUMB_UPLOAD_DIR' ) ) {
                ...
                $length_img_path = strlen( $image_name_path );
                if($length_img_path<7){
                    $upload_dir .= "/thumbs";
                    $upload_url .= "/thumbs";
                }else{
                    $upload_dir .= "/thumbs/". substr($image_name_path,0,2)."/".substr($image_name_path,2,2);
                    $upload_url .= "/thumbs/". substr($image_name_path,0,2)."/".substr($image_name_path,2,2);
                }
            }
